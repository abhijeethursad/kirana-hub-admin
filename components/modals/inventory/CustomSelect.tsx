  ...
  return (
    <div className="relative w-full z-50" ref={containerRef}>
      <button 
        ...
        className={`w-full flex items-center justify-between px-4 py-3 text-left rounded-xl border transition-[box-shadow] duration-200 active:scale-[0.98] ${
          isOpen 
      ...
      >
        ...
        {/* 🚀 Kept transition-transform for the flip animation */}
        <ChevronDownIcon className={`h-4 w-4 text-slate-500 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
      </button>
      
      {isOpen && (
        <div className="absolute z-100 mt-2 w-full bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95 duration-100">
          <div className="p-1.5 flex flex-col gap-0.5">
            {options.map((opt) => (
              <div 
                key={opt} 
                onClick={() => { onChange(opt); setIsOpen(false); }} 
                className={`flex items-center justify-between px-3 py-3 sm:py-2.5 rounded-lg cursor-pointer text-sm font-medium ${
                  value === opt 
