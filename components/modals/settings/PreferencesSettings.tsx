...
export default function PreferencesSettings() {
  ...
  const handleThemeChange = (newTheme: 'light' | 'dark' | 'system', e: React.MouseEvent) => {
    setTheme(newTheme, e);
    broadcastSync('THEME_CHANGED', newTheme); // 🚀 1 Line!
  };
      ...
      <div className="space-y-8">
        <div>
          ...
          <div className="grid grid-cols-3 gap-1 sm:gap-2 bg-slate-50 dark:bg-slate-900/40 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-inner dark:shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)]">
            <button onClick={(e) => handleThemeChange('light', e)} className={`${baseBtnClass} ${theme === 'light' ? activeBtnClass : inactiveBtnClass}`}>
            ...
            <button onClick={(e) => handleThemeChange('dark', e)} className={`${baseBtnClass} ${theme === 'dark' ? activeBtnClass : inactiveBtnClass}`}>
            ...
            <button onClick={(e) => handleThemeChange('system', e)} className={`${baseBtnClass} ${theme === 'system' ? activeBtnClass : inactiveBtnClass}`}>
