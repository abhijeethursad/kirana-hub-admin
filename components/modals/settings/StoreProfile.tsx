...
export default function StoreProfile({ profile, store, setStore, handlePhotoUpload, isUploadingLogo, onToggleStatus, onSave, isSaving, isDirty }: StoreProfileProps) {
  ...
  useEffect(() => {
    // Only listen locally, GlobalSync handles the rest!
    const handleSync = (e: CustomEvent) => {
      setStore({ ...storeRef.current, isOpen: e.detail });
    };
  ...
  const handleLocalToggle = () => {
    const newState = !store.isOpen;
    
    setStore({ ...store, isOpen: newState });
  ...
  return (
    <div className="bg-white dark:bg-slate-800/90 backdrop-blur-xl p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-700/60 shadow-sm dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_4px_20px_rgba(0,0,0,0.1)] relative">
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <button onClick={handleLocalToggle} className={`px-3 sm:px-4 py-2 rounded-full text-xs font-extrabold tracking-wide border flex items-center gap-2 transition-transform duration-300 active:scale-95 shadow-sm dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] ${store.isOpen ? "bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-500/15 dark:border-emerald-500/30 dark:text-emerald-400" : "bg-red-50 border-red-200 text-red-700 dark:bg-red-500/15 dark:border-red-500/30 dark:text-red-400"}`}>
      ...
      {/* Inputs Section (Kept Exactly the same) */}
      <div className="flex flex-col sm:flex-row gap-6 mb-6">
        <div className="shrink-0 flex sm:items-center justify-center">
          ...
          <div onClick={() => !isUploadingLogo && fileInputRef.current?.click()} className={`relative h-28 w-28 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border-2 border-dashed border-slate-300 dark:border-slate-700 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 dark:hover:border-indigo-500/60 group overflow-hidden shadow-inner dark:shadow-[inset_0_2px_6px_rgba(0,0,0,0.3)] ${isUploadingLogo ? 'opacity-50' : ''}`}>
            {store.logo ? ( <img src={store.logo} alt="Logo" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" /> ) : isUploadingLogo ? ( <ArrowPathIcon className="h-6 w-6 text-indigo-500 animate-spin" /> ) : ( <> <PhotoIcon className="h-8 w-8 text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 dark:group-hover:text-indigo-400" /> <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-widest group-hover:text-indigo-500">Upload</span> </> )}
          </div>
        </div>
      ...
      <div className={`transition-[max-height,opacity,margin,padding] duration-500 ease-in-out overflow-hidden ${isDirty ? 'max-h-24 opacity-100 mt-8 pt-6 border-t border-slate-100 dark:border-slate-800' : 'max-h-0 opacity-0 m-0 p-0 border-transparent'}`}>
        <div className="flex justify-end">
          <button onClick={onSave} disabled={isSaving || isUploadingLogo} className="px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-transform active:scale-95 disabled:opacity-50 text-white bg-gradient-to-b from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 shadow-lg shadow-indigo-500/25 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_4px_10px_rgba(99,102,241,0.2)] text-sm">
